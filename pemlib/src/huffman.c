/*
 * huffman.c - Huffman encoding table initialization
 *
 ...
 *
 * This file initializes the Huffman encoding tables used for compressing
 * quantized MDCT coefficients. MP3 uses 34 different Huffman tables
 * optimized for different amplitude distributions.
 *
 * Tables 0-15: For coefficients with maximum values 0-15
 * Tables 16-23: For coefficients with escape codes (linbits)
 * Tables 24-31: Alternate tables for large values
 * Tables 32-33: For count1 region (quadruples of 0/1 values)
 *
 * The hbits array pre-computes the total bit cost for each (x, y) pair
 * in each table, including sign bits and linbits where applicable.
 *
