 ...
 *
 * This file implements the quantization loop that converts MDCT coefficients
 * to integer values and performs rate control to meet the target bitrate.
 *
 * The quantization process:
 *   1. Apply global gain to scale coefficients
 *   2. Quantize using power-law: ix = floor(|xr| ^ 0.75)
 *   3. Count bits required for Huffman encoding
 *   4. Binary search for optimal gain to hit target bits
 *   5. Adjust scalefactors if quantization fails
 *
 * Rate control parameters:
 *   - HA, HB, HC: Hunt step sizes for gain search
 *   - MIN_GAIN, MAX_GAIN: Valid gain range (150-255)
 *   - CUT: Bit allocation threshold
 *
 * See LICENSE for terms of use.
 */

/* Hunt algorithm step sizes for binary search. */
#define HA 3
#define HB 3
#define HC 6
