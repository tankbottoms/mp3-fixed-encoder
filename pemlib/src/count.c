/*
 * count.c - Bit counting for Huffman encoding
 *
 ...
 *
 * This file implements bit counting for the Huffman encoding stage.
 * After quantization, the encoder must determine:
 *   1. How to divide coefficients into regions
 *   2. Which Huffman table to use for each region
 *   3. Total bit count for the encoded granule
 *
 * Region structure (for long blocks):
 *   - Region 0: Low-frequency coefficients (pos0)
 *   - Region 1: Mid-frequency coefficients (pos0 to pos1)
 *   - Region 2: High-frequency coefficients (pos1 to pos2, "big values")
 *   - Count1: Trailing coefficients with values 0 or 1 (pos2 to pos3)
 *
 * Key functions:
 *   - ix_max(): Find maximum quantized value in a region
 *   - choose_table(): Select optimal Huffman table for a region
 *   - count_bits(): Calculate total bits for a granule
 *   - subdivide(): Determine region boundaries
 *
...
/* Find maximum quantized value in a region. */
int ix_max(int begin, int end)
{
	int i;
	int x;
	short *pix;
