 ...
 *
 * This file implements the polyphase analysis filter bank that transforms
 * PCM audio samples into 32 subband signals. The filter bank is the first
 * stage of MP3 encoding, dividing the audio spectrum into frequency bands
 * for independent processing.
 *
 * The filter uses a 512-tap window function applied to a sliding buffer
 * of input samples, followed by a 32-point DCT to produce subband outputs.
 *
 * Key optimizations:
 *   - Fixed-point arithmetic using cfm() for 32x32->64->high32 multiply
 *   - Pre-computed window coefficients (iwinn) stored in ROM
 *   - Circular buffer management for efficient sample handling
 *
...
/*
 * cfm - Fixed-point multiply returning high 32 bits of 64-bit product.
 *
 * Fixed for LP64 systems (macOS ARM64, Linux x86_64) where 'long' is 64 bits.
 * The original union-based approach failed because it assumed 'long' was 32 bits.
 */
static inline int cfm(int a1, int a2) {
	int64_t product = (int64_t)a1 * a2;
#ifdef PEM_PRECISION
	/* Round toward zero for better precision in psychoacoustic model. */
...
/* GET2h macro: Read two 16-bit values from a pointer and advance it
 * Fixed for modern C compilers (removed lvalue cast)
 */
#ifdef PEM_ARM_ASM
...
#else
/* Use simple path - read two shorts sequentially */
#define GET2h(x, a, b) do { \
	a = *x++; b = *x++; \
} while(0)
#endif
