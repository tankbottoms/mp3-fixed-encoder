/*
 * memory.h - Memory layout and accessor macros
 *
 ...
 *
 * This file defines the memory layout for the MP3 encoder, separating
 * data into fast SRAM (sram) and slower DRAM (dram) structures.
 *
 * On embedded targets:
 *   - struct sram (~40 KB): Placed in on-chip SRAM for fast access
 *   - struct dram (~20 KB): Placed in external DRAM
 *   - struct ro_sram: Read-only tables, typically in ROM
 *
 * The FASTVAR() macro accesses SRAM variables through sram_p pointer,
 * allowing the compiler to optimize memory access patterns.
 *
 * LP64 fix: Changed bit buffer types (sbuffer, dbuffer, mainbitter,
 * sidebitter) from unsigned long to uint32_t for consistent 32-bit
...
struct sram {
	...
	/* Fixed for LP64: use uint32_t instead of unsigned long for 32-bit buffers */
	uint32_t *sbuffer, *dbuffer;
	uint32_t sbufferword, dbufferword;
...
#endif
	/* Fixed for LP64: use uint32_t for 32-bit bit buffer arrays */
	uint32_t mainbitter[4096];
	uint32_t sidebitter[72];
	jmp_buf error_jbuf;
} dram;
