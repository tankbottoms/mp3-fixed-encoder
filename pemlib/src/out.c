/*
 * out.c - Bitstream formatting and output
 *
 ...
 *
 * This file implements the bitstream writer for MP3 frames:
 *   - Side information encoding (32 bytes per frame)
 *   - Huffman-coded main data with bit reservoir
 *   - Byte-aligned frame output
 *
 * The bitstream is double-buffered:
 *   - sbitter: Side information accumulator
 *   - dbitter: Main data accumulator
 *
 * LP64 fix: Changed buffer types from 'unsigned long' to 'uint32_t'
 * to ensure correct 32-bit word operations on 64-bit platforms.
 *
...
/*
 * pem_do_out - Write bytes to the output buffer.
 */
...
FAST_FUNC void new_sbitter(uint32_t *buf)
{
	FASTVAR(sbuffer) = buf;
	FASTVAR(sbits_left) = 32;
}

FAST_FUNC void new_dbitter(uint32_t *buf)
{
	FASTVAR(dbuffer) = buf;
	FASTVAR(dbits_left) = 32;
}

/* Fixed for LP64: explicitly use uint32_t for 32-bit byte swap */
FAST_FUNC uint32_t SWAP(uint32_t x)
{
#ifdef BIGENDIAN
	return x;
#else
...
{
#ifdef PEM_DEBUG
	static int dbitter_count = 0;
	if (dbitter_count < 10) {
		fprintf(stderr, "  dbitter[%d]: x=%lu n=%d bits_left=%d dbuffer=%p mainbitter=%p\n",
			dbitter_count, x, n, FASTVAR(dbits_left), (void*)FASTVAR(dbuffer), (void*)mainbitter);
		dbitter_count++;
	}
#endif
...
FAST_FUNC int tell_dbitter(uint32_t *buf)
{
...
FAST_FUNC int flush_dbitter(uint32_t *buf)
{
	int no;
...
void out_start(void)
{
	...
	/* Sample rate index (2 bits) + padding (1 bit) + private (1 bit) */
	sbitter(FASTVAR(mpeg).sample_rate_idx << 2, 4);
